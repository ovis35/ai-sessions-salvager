...
`safe_id` is generated by `safe_id()` in `convert_and_analyze.py`: any character outside `[a-zA-Z0-9_.-]` is replaced with `_`, and then the result is truncated to at most 80 characters.
